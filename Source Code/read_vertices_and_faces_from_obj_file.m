function [V,F] = read_vertices_and_faces_from_obj_file(filename)
  ...
  while ischar(line)
    ...
    face_long_mid = sscanf(line,'f %d/%d %d/%d %d/%d',6);
   ...
   % see if line is simple face command if so add to faces
    ...
    elseif(size(face_long,1)==6)
      ...
      face_index = face_index+1;
      % see if line is a face with single slash i.e d/d d/d d/d
    elseif(size(face_long_mid,1)==6)
      % remove normal
      face_long_mid = face_long_mid(1:2:end);
      F(face_index,:) = face_long_mid;
      face_index = face_index+1;
